  ...
  namespace :install do
    ...
    task migrations: :'db:load_config' do
      ...
      if ENV["DATABASE"].present? && ENV["DATABASE"] != 'primary'
        config = ActiveRecord::Base.configurations.configs_for(spec_name: ENV["DATABASE"])
        raise "Invalid DATABASE provided" if config.blank?
        destination = config.migrations_paths
        raise "#{ENV['DATABASE']} does not have custom migration paths" if destination.blank?
      end
